...
/** Roughly the width of a short month name plus breathing room. */
...
export const HeatmapXAxis = memo(function HeatmapXAxis({
  ...
  const labels = useMemo(() => {
    ...
    for (let columnIndex = 0; columnIndex < data.length; columnIndex++) {
      ...
      const x = margin.left + xScale(columnIndex);
      // A month whose first column sits right next to the previous label
      // overlaps it ("AğuEyl"): the calendar starts mid-month, so column 0 and
      // the next month boundary can be one cell apart. Drop the later one —
      // the grid is still anchored, and two overlapping words label nothing.
      const previous = ticks[ticks.length - 1];
      if (previous && x - previous.x < MIN_LABEL_GAP_PX) {
        lastMonthKey = monthKey;
        continue;
      }

      ticks.push({ label: monthFmt.format(monthAnchor), x, key: monthKey });
      lastMonthKey = monthKey;
    }
