...
export interface SankeyNodeProps {
  ...
  /** Unit appended to the value label. The registry hardcoded "sessions",
   * which is only right for its own demo data — a sankey whose links are
   * tool calls or dollars would have been labelled with the wrong noun. */
  valueUnit?: string;
  /**
      ...
      {showLabels ? (
        <>
          ...
          {labelLayouts.value ? (
            ...
            >
              {valueUnit ? `${intFmt(value)} ${valueUnit}` : intFmt(value)}
            </NodeLabel>
          ) : null}
        </>
      ) : null}
