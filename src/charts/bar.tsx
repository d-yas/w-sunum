...
function AnimatedBar({
  ...
  // Horizontal bars grow with `scaleX`, not by animating the `width`
  // attribute. The easing undershoots slightly below its start value, and a
  // width animated from 0 therefore passes through negative numbers — which
  // SVG rejects outright ("<rect> attribute width: A negative value is not
  // valid"), one console error per frame per bar. A transform has no such
  // constraint and gives the identical motion.
  if (isHorizontal) {
    return (
      ...
      {data.map((d, i) => {
        ...
        if (isHorizontal) {
          ...
          if (stacked && stackOffsets) {
            ...
            x = scale(offset) ?? 0;
            // The segment ends at `offset + value`, not at `value`. Using
            // `valuePos` here made every stacked series after the first come
            // out negative-width (scale(3) - scale(30)), which SVG rejects —
            // the whole chart rendered as one bar plus console errors. The
            // vertical path does not have this bug: it derives a pixel
            // *height* first and stacks upward from the offset.
